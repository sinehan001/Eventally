...
<head>
  ...
  <meta charset="utf-8">
  <title>Events Results - Eventally</title>
  <style>
        body
        {
...
<?php
          ...
          $dbname="eventally";
  $conn=mysqli_connect($dbserver,$dbuser,$dbpass,$dbname);  
  $result=mysqli_query($conn,"SELECT * from result ORDER by id DESC"); 
       while($row = mysqli_fetch_assoc($result))
       {
        date_default_timezone_set("Asia/Kolkata");
        echo'<div class="container container-center">';
        echo'    <img src="img/result/'.$row['image'].'">';
        echo'    <h2 style="color: red;"> '.$row['title'].'</h2>';
        echo'    <h3 style="color: blue;"> '.$row['dates'].' </h3>';
        echo'    <h2> 1<sup>st</sup> - '.$row['winners'].' </h2>';
        echo'    <h2> 2<sup>nd</sup> - '.$row['winner1'].' </h2>';
        echo'    <h2> 3<sup>rd</sup> - '.$row['winner2'].' </h2>';
        echo'</div>';
       }
?>
<br>
